...
config.h:
	cp config.def.h $@
clean:
	rm -f dwl *.o *-protocol.h tests/swallow

dist: clean
	mkdir -p dwl-$(VERSION)
	cp -R Makefile CHANGELOG.md README.md client.h config.def.h \
		config.mk protocols tests dwl.1 dwl.c util.c util.h dwl.desktop \
		dwl-$(VERSION)
